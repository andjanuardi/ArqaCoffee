      ...
      '<div class="w-full max-w-md text-center py-8">',
        ...
        '<div class="space-y-3 mb-6">',
          '<div class="flex items-center justify-between p-4 rounded-xl" style="background:var(--card)">',
            ...
            '</div>',
            '<div class="text-2xl ' + (geoOk ? 'text-green-400' : 'text-red-400') + '"><i class="fas ' + (geoOk ? 'fa-check-circle' : 'fa-times-circle') + '"></i></div>',
          '</div>',
          '<div class="flex items-center justify-between p-4 rounded-xl" style="background:var(--card)">',
            '<div class="flex items-center gap-3">',
            ...
            '</div>',
            '<div class="text-2xl ' + (camOk ? 'text-green-400' : 'text-red-400') + '"><i class="fas ' + (camOk ? 'fa-check-circle' : 'fa-times-circle') + '"></i></div>',
          '</div>',
          '<div class="flex items-center justify-between p-4 rounded-xl" style="background:var(--card)">',
            '<div class="flex items-center gap-3">',
            ...
            '</div>',
            '<div class="text-2xl ' + (notifOk ? 'text-green-400' : 'text-red-400') + '"><i class="fas ' + (notifOk ? 'fa-check-circle' : 'fa-times-circle') + '"></i></div>',
          '</div>',
        '</div>',
        bothOk
        ...
        bothOk ? '' : '<div class="text-xs px-4 leading-relaxed" style="color:var(--muted)"><i class="fas fa-info-circle mr-1"></i>Jika akses ditolak, buka <b>Pengaturan Browser</b> → <b>Privasi & Keamanan</b> → Izinkan akses <b>Lokasi</b>, <b>Kamera</b>, dan <b>Notifikasi</b>, reload halaman lalu klik tombol di atas</div>',
      '</div>',
    '</div>',
  ].join('');
...
function checkRemainingSilently() {
  ...
  checkPermission('geolocation',
    function() { State._geoGranted = true; pending--; done(); },
    function() { allGranted = false; pending--; done(); }
  );

  // Camera
  pending++;
  checkPermission('camera',
    function() { State._cameraGranted = true; pending--; done(); },
    function() { allGranted = false; pending--; done(); }
  );
...
function checkPermission(name, onGranted, onDenied) {
  if (navigator.permissions && navigator.permissions.query) {
    navigator.permissions.query({name: name}).then(function(result) {
      ...
      else { requestActualAPI(name, onGranted, onDenied); }
    }).catch(function() { requestActualAPI(name, onGranted, onDenied); });
  } else {
...
function requestActualAPI(name, onGranted, onDenied) {
  if (name === 'geolocation') {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) { onGranted(); return; }
    navigator.mediaDevices.getUserMedia({video: true}).then(function(stream) {
      stream.getTracks().forEach(function(t) { t.stop(); });
      onGranted();
    }).catch(function(err) {
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') onDenied();
      else onGranted();
    });
  } else {
    onGranted();
...
function requestBothPermissions() {
  ...
  if (navigator.geolocation) {
    total++;
    navigator.geolocation.getCurrentPosition(
      function() { State._geoGranted = true; done++; if (done === total) checkPermissions(); },
      function(err) {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.mediaDevices.getUserMedia({video: true}).then(function(stream) {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') State._cameraGranted = false;
