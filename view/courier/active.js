...
// ============================================================
async function renderCourierActive() {
  try {
    showSkeleton('courier-active', 'orders');
      ...
      <div class="space-y-3">
        ...
        <div class="card">
          ...
          <div class="p-3 rounded-xl mb-3" style="background:var(--bg2)">
            ...
            ${(() => { var d = getCafeToCustDist(o); return d ? `<div class="flex justify-between text-xs"><span style="color:var(--muted)">Cafe → Pelanggan</span><span style="color:var(--accent)">${d}</span></div>` : ''; })()}
          </div>
          ${(() => {
            var mitraItems = o.items.filter(i => {
              var mi = getMenuItem(i.menu_item_id);
              return mi && mi.submitted_by;
            });
            var mitraNames = [...new Set(mitraItems.map(i => getMenuItem(i.menu_item_id).submitted_by))];
    var mitraPosHtml = mitraNames.map(name => {
      var pos = State.mitraPositions[name] || (DB.users || []).find(u => u.name === name)?.mitra_position;
              var distHtml = '';
              if (pos && cafe) {
                var d = calcDistance(cafe.location.lat, cafe.location.lng, pos.lat, pos.lng);
                var meter = Math.round(d).toLocaleString('id-ID');
                var km = (d / 1000).toFixed(1).replace('.', ',');
  ...
  } catch(e) { console.error(e); showToast('Gagal memuat pengantaran aktif', 'error'); return ''; }
  finally { hideSkeleton('courier-active'); }
}

function showNavigationModal(orderId) {
  var o = DB.orders.find(x => x.id === orderId);
  if (!o) return;
  var courierPos = State.courierPosition || DB.cafe?.location;
  if (!courierPos) { showToast("Posisi kurir tidak diketahui", "error"); return; }
  var origin = `${courierPos.lat},${courierPos.lng}`;
  var dest = o.delivery_location?.lat ? `${o.delivery_location.lat},${o.delivery_location.lng}` : null;
  var mitraList = [...new Set((o.items || []).map(i => {
    var mi = getMenuItem(i.menu_item_id);
  ...
  var hasDest = !!dest;
  var hasMitra = mitraList.some(m => m.pos);
  var waypoints = mitraList.filter(m => m.pos).map(m => `${m.pos.lat},${m.pos.lng}`);
  var routeUrl = waypoints.length > 0 && hasDest
    ...
    : null;
  var posSayaDist = courierPos && DB.cafe ? (function() {
    var d = calcDistance(courierPos.lat, courierPos.lng, DB.cafe.location.lat, DB.cafe.location.lng);
    var m = Math.round(d).toLocaleString('id-ID');
...
function focusMitraOnMap(orderId, mitraName) {
  var map = State.mapInstances[orderId];
  var pos = State.mitraPositions[mitraName] || DB.users.find(u => u.name === mitraName)?.mitra_position;
  if (!map || !pos) return;
...
async function completeDelivery(id) {
  try {
    var btn = document.querySelector('button[onclick="completeDelivery(\'' + id + '\')"]');
    ...
    if (o.payment_status === "unpaid" && !["digital", "qris", "bank_transfer"].includes(o.payment_method)) {
      ...
      await API.updateOrder(id, { status: "completed", ongkir_status: "unpaid" });
      notifyDeliveryCompleted(o);
      showToast("Pengantaran selesai! Ongkir " + formatCurrency(o.shipping_cost) + " dari kasir — jangan lupa ambil ongkir", "info");
    }
    await render();
  } catch(e) { console.error(e); showToast('Gagal menyelesaikan pengantaran', 'error'); }
