...
// ============================================================
async function renderCourierView() {
  try {
    showSkeleton('courier-view', 'courier');
    var tab = State.currentTab.courier || "available";
    if (tab === "available") return await renderCourierAvailable();
    if (tab === "active") return await renderCourierActive();
    if (tab === "history") return await renderCourierHistory();
    if (tab === "profile") return await renderCourierProfile();
    return await renderCourierAvailable();
  } catch(e) { console.error(e); showToast('Gagal memuat tampilan kurir', 'error'); return ''; }
  finally { hideSkeleton('courier-view'); }
}

async function renderCourierAvailable() {
  try {
    showSkeleton('courier-available', 'orders');
    var orders = await API.getOrders();
    var attendances = await API.getAttendances();
    var cafe = await API.getCafe();
    var hasActive = orders.some(o => o.courier_id === State.currentUser.id && o.status === "delivering");
    ...
    function getCafeToCustDist(o) {
      ...
      var _cafe = cafe.location;
      var d = calcDistance(_cafe.lat, _cafe.lng, o.delivery_location.lat, o.delivery_location.lng);
    ...
    <div class="animate-fade-up">
      ...
      <div class="space-y-3">
        ...
        <div class="card">
          ...
          ${(() => { var fee = calcCourierFee(o.shipping_cost); return fee > 0 ? `<div class="text-xs mb-1" style="color:var(--muted)"><i class="fas fa-hand-holding-dollar mr-1"></i>Jasa Aplikasi: <b style="color:var(--danger)">-${formatCurrency(fee)}</b></div>` : ''; })()}
          ${o.payment_status === "paid" && o.shipping_cost > 0 ? `<div class="text-xs mb-1" style="color:var(--success)"><i class="fas fa-hand-holding-dollar mr-1"></i>Ongkir dari kasir: <b>${formatCurrency((o.shipping_cost || 0) - calcCourierFee(o.shipping_cost))}</b></div>` : ""}
  ...
  } catch(e) { console.error(e); showToast('Gagal memuat pesanan tersedia', 'error'); return ''; }
  finally { hideSkeleton('courier-available'); }
}

async function courierCheckIn() {
  try {
...
async function courierCheckOut() {
  var today = new Date().toLocaleDateString('sv-SE');
  var att = DB.attendances.find(function(a) { return a.user_id === State.currentUser.id && !a.check_out && new Date(a.check_in).toLocaleDateString('sv-SE') === today; });
...
function updateCourierPosDisplay(lat, lng) {
  var cl = document.getElementById('courier-pos-coords');
  var dl = document.getElementById('courier-pos-distance');
  if (cl) cl.textContent = lat.toFixed(5) + ', ' + lng.toFixed(5);
  if (dl && DB.cafe) {
    var d = calcDistance(lat, lng, DB.cafe.location.lat, DB.cafe.location.lng);
    var meter = Math.round(d).toLocaleString('id-ID');
    var km = (d / 1000).toFixed(1).replace('.', ',');
    if (d < 1000) {
...
async function renderCourierProfile() {
  try {
    showSkeleton('courier-profile', 'profile');
    var attendances = await API.getAttendances();
    ...
    var today = new Date().toLocaleDateString('sv-SE');
    var att = attendances.find(a => a.user_id === u.id && !a.check_out && new Date(a.check_in).toLocaleDateString('sv-SE') === today);
    var cafe = cafeData.location;
    return `
    <div class="animate-fade-up">
  ...
  finally { hideSkeleton('courier-profile'); }
}
...
function rejectCourierOrder(orderId) {
  var o = DB.orders.find((x) => x.id === orderId);
  if (!o) return;
  showModal(`
