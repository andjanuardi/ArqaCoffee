...
function renderBottomNav(role) {
  const tabs = {
    ...
    cashier: [{ id: 'orders', icon: 'fa-clipboard-list', label: 'Pesanan' }, { id: 'payment', icon: 'fa-check-circle', label: 'Selesai' }, { id: 'report', icon: 'fa-chart-bar', label: 'Laporan' }, { id: 'tables-mgmt', icon: 'fa-table-cells', label: 'Meja' }, { id: 'profile', icon: 'fa-user', label: 'Profil' }],
  ...
  return `<nav class="bottom-nav">${items.map(t => `
    <div class="nav-item ${active === t.id ? 'active' : ''}" onclick="switchTab('${t.id}')">
      ...
      ${t.id === 'orders' && role === 'cashier' && (DB.orders.some(o => o.payment_status === 'unpaid' && o.status !== 'cancelled' && o.status !== 'rejected') || getMitraPendingPayouts().length > 0) ? `<span class="notif-dot"></span>` : ''}
    </div>`).join('')}
  </nav>`;
}
