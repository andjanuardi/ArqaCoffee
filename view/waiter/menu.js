...
async function renderWaiterView() {
  ...
  var tab = State.currentTab.waiter || "menu";
...
async function renderWaiterMenu() {
  if (!isCheckedIn()) {
    return `
  ...
  showSkeleton('waiter-menu', 'menu');
  try {
    var menuData = await API.getMenu();
    var usersData = await API.getUsers();
    var cats = [
      { id: "all", label: "Semua" },
    ...
    var items = menuData.filter(function(m) { return m.is_available && m.is_approved !== false; });
    if (State.selectedCategory !== "all")
      items = items.filter(function(m) { return m.category === State.selectedCategory; });
    if (State.searchQuery)
      items = items.filter(function(m) {
        return m.name.toLowerCase().indexOf(State.searchQuery.toLowerCase()) !== -1;
      });
    hideSkeleton('waiter-menu');
    return `
    <div class="animate-fade-up">
      <div class="mb-6">
      ...
      ${
        State.activePromoId
          ? (function() {
              var activeP = DB.promos.find(function(x) { return x.id === State.activePromoId; });
              return activeP ? `
      ...
      ${
        DB.promos && DB.promos.filter(function(p) { return p.is_active && isPromoActiveByDate(p) && !isPromoUsedByUser(p.id) && p.id !== State.activePromoId; }).length > 0
          ? `
      <div class="promo-carousel" id="promo-carousel">
        <div class="promo-track" id="promo-track">
          ${DB.promos
            .filter(function(p) { return p.is_active && isPromoActiveByDate(p) && !isPromoUsedByUser(p.id) && p.id !== State.activePromoId; })
            .map(
              function(p) {
                var discLabel = p.discount_type === 'fixed' ? formatCurrency(p.discount_value) : p.discount_value + '%';
                var bgStyle = p.image ? "background:linear-gradient(135deg,rgba(0,0,0,.6),rgba(0,0,0,.3)),url('" + p.image + "') center/cover" : "background:linear-gradient(135deg," + (p.color || '#E07A3A') + "," + (p.color || '#E07A3A') + "dd)";
                return `
        ...
        ${DB.promos.filter(function(p) { return p.is_active && isPromoActiveByDate(p) && !isPromoUsedByUser(p.id) && p.id !== State.activePromoId; }).length > 1 ? '<div class="promo-dots" id="promo-dots"></div>' : ''}
      </div>`
          : ""
      }
      <div class="flex gap-2 mb-5 overflow-x-auto pb-2" style="-webkit-overflow-scrolling:touch;scrollbar-width:none;">
        ${cats.map(function(c) { return '<div class="category-chip ' + (State.selectedCategory === c.id ? "active" : "") + '" onclick="State.selectedCategory=\'' + c.id + '\';render()">' + c.label + '</div>'; }).join("")}
      </div>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        ${items
          .map(
            function(m) {
              var hasPromo = State.activePromoId && (function() {
                var p = DB.promos.find(function(x) { return x.id === State.activePromoId; });
                if (!p || !p.is_active) return false;
                return !p.menu_ids || !p.menu_ids.length || p.menu_ids.indexOf(m.id) !== -1;
              })();
              return `
    ...
    return '<div class="text-center py-12"><p style="color:var(--muted)">Gagal memuat menu</p></div>';
  }
}

async function renderWaiterCart() {
  if (State.cart.length === 0) return renderWaiterEmptyCart();
  var activePromo = getActivePromo();
  var total = State.cart.reduce(function(s, c) { return s + c.unit_price * c.quantity; }, 0);
  ...
  <div class="animate-fade-up">
    ...
    <div class="space-y-3 mb-6">${State.cart.map(function(c, i) {
      var eligible = isItemEligible(c, activePromo);
      var discUnitPrice = calcItemDiscount(c, activePromo);
    ...
    <div class="card mb-4">
      ...
      ${State.selectedTable
        ? '<div class="flex items-center gap-2 p-3 rounded-xl text-xs" style="background:rgba(39,174,96,.1);color:var(--success)"><i class="fas fa-check-circle"></i><span>Meja <b>' + (getTable(State.selectedTable)?.number || "-") + '</b> dipilih</span><button onclick="State.selectedTable=null;render()" class="ml-auto text-xs underline" style="color:var(--muted)">Ganti</button></div>'
        : '<button onclick="showWaiterTableSelector()" class="btn-secondary w-full text-center flex items-center justify-center gap-2"><i class="fas fa-chair"></i> Pilih Meja</button>'}
    </div>
    <div class="card mb-4">
      ...
      ${State.payTiming !== "later"
        ? '<label class="text-xs font-semibold mb-3 block" style="color:var(--muted)">Metode Pembayaran</label><div class="grid grid-cols-3 gap-3"><div class="card text-center py-3 cursor-pointer text-sm" onclick="selectPayment(\'qris\')" style="' + (selectedPayment === "qris" ? "border-color:var(--accent);background:rgba(224,122,58,.08)" : "") + '"><i class="fas fa-qrcode mb-1" style="color:var(--accent)"></i><br>QRIS</div><div class="card text-center py-3 cursor-pointer text-sm" onclick="selectPayment(\'bank_transfer\')" style="' + (selectedPayment === "bank_transfer" ? "border-color:var(--accent);background:rgba(224,122,58,.08)" : "") + '"><i class="fas fa-university mb-1" style="color:var(--accent)"></i><br>Transfer</div><div class="card text-center py-3 cursor-pointer text-sm" onclick="selectPayment(\'cash\')" style="' + (selectedPayment === "cash" ? "border-color:var(--accent);background:rgba(224,122,58,.08)" : "") + '"><i class="fas fa-money-bill mb-1" style="color:var(--success)"></i><br>Tunai</div></div>'
        : '<div class="flex items-center gap-2 p-3 rounded-xl text-xs" style="background:rgba(243,156,18,.1);color:var(--warning)"><i class="fas fa-info-circle"></i><span>Pesanan akan dibuat dengan status <b>Belum Bayar</b>. Silakan arahkan pelanggan ke kasir.</span></div>'}
    </div>
...
function renderWaiterCartItem(c, i, eligible, discUnitPrice) {
  var mi = getMenuItem(c.menu_item_id);
  var taxRate = mi ? mi.tax_percentage : 0;
  return `
  <div class="card">
...
function renderWaiterPromoBanner(activePromo) {
  return '<div class="flex items-center gap-2 mb-3 text-xs p-3 rounded-xl" style="background:rgba(39,174,96,.1);color:var(--success)"><i class="fas fa-tag"></i><span class="flex-1">Promo <b>' + activePromo.title + '</b> aktif</span><button onclick="State.activePromoId=null;render()" class="text-xs underline" style="color:var(--muted)">Batalkan</button></div>';
}

function renderWaiterEmptyCart() {
  return '<div class="text-center py-16 animate-fade-up"><i class="fas fa-shopping-bag text-5xl mb-4" style="color:var(--border)"></i><h3 class="semibold text-lg mb-2">Keranjang Kosong</h3><p class="text-sm mb-6" style="color:var(--muted)">Tambahkan menu dari daftar menu</p><button onclick="switchTab(\'menu\')" class="btn-primary">Lihat Menu</button></div>';
}

async function showWaiterTableSelector() {
  try {
    ...
    showModal(`
      <div class="text-center">
        ...
        <div class="grid grid-cols-4 gap-3 mb-6">
          ${tablesData
            .map(
              function(t) { return '<button class="card text-center py-3 text-sm font-semibold ' + (t.status === "occupied" ? "opacity-40 cursor-not-allowed" : "") + '" onclick="' + (t.status === "available" ? "selectWaiterTable('" + t.id + "')" : "") + '" style="' + (t.status === "occupied" ? "pointer-events:none" : "") + '"><i class="fas fa-chair mb-1" style="color:' + (t.status === "available" ? "var(--success)" : "var(--danger)") + '"></i><br>' + t.number + (t.status === "occupied" ? '<br><span style="font-size:9px;color:var(--danger)">Terisi</span>' : '<br><span style="font-size:9px;color:var(--success)">Kosong</span>') + '</button>'; },
            )
            .join("")}
        </div>
    ...
    console.error(e);
    showToast("Gagal memuat daftar meja", "error");
  }
}
...
async function confirmWaiterPlaceOrder() {
  ...
  var itemsList = State.cart.map(function(c) {
    return c.menu_item.name + " x" + c.quantity + " = " + formatCurrency(c.unit_price * c.quantity);
  }).join('</div><div class="text-sm" style="color:var(--muted)">');
  var activePromo = State.activePromoId ? DB.promos.find(function(x) { return x.id === State.activePromoId; }) : null;
...
async function placeWaiterOrder() {
  ...
  try {
    ...
    var order = {
      ...
      items: State.cart.map(function(c) { return {
...
async function renderWaiterOrders() {
  showSkeleton('waiter-orders', 'list');
  try {
    var ordersData = await API.getOrders();
    var myOrders = ordersData.filter(function(o) {
      if (o.user_id === State.currentUser.id) return true;
      if (o.waiter_id === State.currentUser.id) return true;
      if (o.order_type === 'dine-in' && ['ready', 'delivered'].indexOf(o.status) !== -1) return true;
      return false;
    });
    var dateFilter = State.waiterOrderDateFilter !== undefined ? State.waiterOrderDateFilter : new Date().toLocaleDateString('sv-SE');
    if (dateFilter) {
      myOrders = myOrders.filter(function(o) { return o.created_at && new Date(o.created_at).toLocaleDateString('sv-SE') === dateFilter; });
    }
    myOrders.sort(function(a, b) { return new Date(b.created_at) - new Date(a.created_at); });
    hideSkeleton('waiter-orders');
    return `
    <div class="animate-fade-up">
    ...
    return '<div class="text-center py-12"><p style="color:var(--muted)">Gagal memuat pesanan</p></div>';
  }
}

async function renderWaiterProfile() {
...
async function serveWaiterOrder(orderId) {
  ...
  try {
    ...
    if (!o || o.order_type !== 'dine-in') { showToast("Pesanan tidak ditemukan", "error"); return; }
    await API.updateOrder(orderId, {
      waiter_id: State.currentUser.id,
      status: 'delivered'
    });
    notifyStatusChange({ id: orderId, order_type: o.order_type, table_id: o.table_id, total_amount: o.total_amount }, 'delivered');
    showToast('Pesanan telah diantarkan ke meja', 'success');
    render();
  } catch (e) {
    console.error(e);
    showToast("Gagal mengantarkan pesanan", "error");
  } finally {
    if (btn) hideBtnSpinner(btn);
  }
}

async function showWaiterPaymentModal(orderId) {
  try {
    var ordersData = await API.getOrders();
    var o = ordersData.find(function(x) { return x.id === orderId; });
...
async function showWaiterTransfer(id) {
  try {
    var ordersData = await API.getOrders();
...
async function processWaiterPayment(id, method) {
  var btn = document.activeElement;
  if (btn) showBtnSpinner(btn);
  try {
    var ordersData = await API.getOrders();
    ...
    if (o.table_id) {
      ...
      var hasOther = allOrders.some(function(x) {
        return x.table_id === o.table_id && x.id !== o.id &&
          ['completed', 'cancelled', 'rejected'].indexOf(x.status) === -1;
      });
      if (!hasOther) {
...
async function waiterCollectCash(id) {
  var btn = document.activeElement;
  if (btn) showBtnSpinner(btn);
  try {
    await API.updateOrder(id, {
    ...
    console.error(e);
    showToast("Gagal mengumpulkan pembayaran", "error");
  } finally {
    if (btn) hideBtnSpinner(btn);
  }
}

async function completeWaiterOrder(orderId) {
  var btn = document.activeElement;
  if (btn) showBtnSpinner(btn);
  try {
    var ordersData = await API.getOrders();
    ...
    notifyStatusChange({ id: orderId, order_type: o.order_type, table_id: o.table_id, total_amount: o.total_amount }, 'completed');
