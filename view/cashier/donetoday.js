...
// ============================================================
async function acceptCashierOrder(id) {
  try {
    ...
    console.error(e);
    showToast("Gagal menerima pesanan", "error");
  }
}

async function processCashPayment(id) {
  try {
...
async function settleDelivery(id) {
  try {
    var updateData = {
      status: "completed",
      payment_status: "paid",
    ...
    console.error(e);
    showToast("Gagal menyelesaikan setoran", "error");
  }
}

async function confirmSettleDineIn(id) {
  try {
    var ordersData = await API.getOrders();
    var o = ordersData.find(function(x) { return x.id === id; });
    if (!o) { showToast("Pesanan tidak ditemukan", "error"); return; }
...
async function cashierSettleDineIn(id) {
  try {
    var ordersData = await API.getOrders();
    var o = ordersData.find(function(x) { return x.id === id; });
    if (!o) return;
...
async function payOngkir(id) {
  try {
    var ordersData = await API.getOrders();
    var o = ordersData.find(function(x) { return x.id === id; });
    if (!o) { showToast("Pesanan tidak ditemukan", "error"); return; }

    await API.updateOrder(id, { ongkir_status: "paid" });
    ...
    console.error(e);
    showToast("Gagal membayar ongkir", "error");
  }
}
...
async function renderCashTable() {
  try {
    var ordersData = await API.getOrders();
    var dateVal = State.cashierReportDate || new Date().toLocaleDateString('sv-SE');
    var orders = ordersData.filter(function(o) {
          ...
          <table class="w-full text-sm" style="border-collapse:collapse">
            ...
            <tbody>${orders.length === 0 ? '<tr><td style="padding:8px 10px;text-align:center;color:var(--muted)" colspan="5">Belum ada transaksi tunai</td></tr>' : orders.map(function(o) {
              ...
              (o.items || []).forEach(function(item) {
                var mi = menuData.find(function(m) { return m.id === item.menu_item_id; });
    ...
    console.error(e);
    showToast("Gagal memuat data tunai", "error");
    return '<div class="card mb-4"><p class="text-center py-4" style="color:var(--muted)">Gagal memuat data</p></div>';
  }
}

async function renderDigitalTable() {
  try {
    var ordersData = await API.getOrders();
    var dateVal = State.cashierReportDate || new Date().toLocaleDateString('sv-SE');
    var orders = ordersData.filter(function(o) {
    ...
    console.error(e);
    showToast("Gagal memuat data digital", "error");
    return '<div class="card mb-4"><p class="text-center py-4" style="color:var(--muted)">Gagal memuat data</p></div>';
  }
}

async function renderCashierReport() {
  showSkeleton('cashier-report', 'report');
  try {
    var ordersData = await API.getOrders();
    var dateVal = State.cashierReportDate || new Date().toLocaleDateString('sv-SE');
    var paidInRange = ordersData.filter(function(o) {
    ...
    var unpaidCount = ordersData.filter(function(o) {
      if (o.payment_status !== 'unpaid' || !o.created_at) return false;
      if (o.status === 'cancelled' || o.status === 'rejected') return false;
      var d = new Date(o.created_at).toLocaleDateString('sv-SE');
      return d === dateVal;
    }).length;
    hideSkeleton('cashier-report');
    return `
    <div class="animate-fade-up">
      ...
      <div class="grid grid-cols-2 gap-3 mb-5">
        ...
        <div class="stat-card cursor-pointer" onclick="State.currentTab['cashier']='payment';render()"><div class="flex items-center gap-2"><i class="fas fa-exclamation-circle" style="color:var(--danger);font-size:18px"></i><span class="text-xs" style="color:var(--muted)">Belum Bayar</span></div><div class="text-xl font-bold mt-1" style="color:var(--danger)">${unpaidCount}</div></div>
      </div>
      ${State.showCashierCashTable ? await renderCashTable() : ''}
      ${State.showCashierDigitalTable ? await renderDigitalTable() : ''}
      ${(function() {
      ...
      <div class="card mb-4">
        ...
        ${(function() {
          var todayOrders = ordersData.filter(function(o) {
          ...
          return '<div class="space-y-2 max-h-72 overflow-y-auto">' + todayOrders.map(function(o) {
            var staffIcon = o.waiter_id && getUser(o.waiter_id) ? '<i class="fas fa-user-tie ml-2" style="color:var(--accent)"></i> ' + getUser(o.waiter_id).name : '';
            var courierIcon = o.courier_id && getUser(o.courier_id) ? '<i class="fas fa-motorcycle ml-2" style="color:var(--accent)"></i> ' + getUser(o.courier_id).name : '';
            return `
    ...
    return '<div class="text-center py-12"><p style="color:var(--muted)">Gagal memuat laporan</p></div>';
  }
}

async function renderCashierProfile() {
