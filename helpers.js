function getOrderTypeName(t) { return t === 'dine-in' ? 'Dine-In' : t === 'takeaway' ? 'Takeaway' : 'Delivery' }
        ...
        async function createMitraPayouts(orderId) {
          ...
          for (var idx = 0; idx < mitraNames.length; idx++) {
            var mitraName = mitraNames[idx];
            ...
            var amount = total - tax - fee;
            if (amount <= 0) continue;
            var exists = DB.mitraPayouts.some(function(p) { return p.order_id === orderId && p.mitra_name === mitraName; });
            if (exists) continue;
            try {
              var payout = await API.createMitraPayout({
              ...
              DB.mitraPayouts.push(payout);
            } catch (e) {
              console.error('[createMitraPayouts]', e);
            }
          }
        }
        ...
        async function syncTableStatus() {
          for (var i = 0; i < DB.tables.length; i++) {
            var t = DB.tables[i];
            var hasActive = DB.orders.some(function(o) {
              return o.table_id === t.id && !['completed', 'cancelled', 'rejected'].includes(o.status);
            });
            var newStatus = hasActive ? 'occupied' : 'available';
            if (t.status !== newStatus) {
              t.status = newStatus;
              try {
                await API.updateTable(t.id, { status: newStatus });
              } catch (e) { console.error('[syncTableStatus]', e); }
            }
          }
        }
