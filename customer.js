// ============================================================
        ...
        function renderCustomerMenu() {
            const cats = [{ id: 'all', label: 'Semua' }, { id: 'coffee', label: 'Kopi' }, { id: 'non-coffee', label: 'Non-Kopi' }, { id: 'food', label: 'Makanan' }, { id: 'snack', label: 'Snack' }];
            ...
            if (State.searchQuery) items = items.filter(m => m.name.toLowerCase().includes(State.searchQuery.toLowerCase()));

            return `
  <div class="animate-fade-up">
    ${State.selectedTable ? `<div class="mb-4 flex items-center gap-2"><span class="badge badge-ready"><i class="fas fa-qrcode mr-1"></i>Meja ${getTable(State.selectedTable)?.number || '-'}</span></div>` : `
    <div class="mb-6">
    ...
    ${!State.selectedTable ? `
    <div class="card mb-6 flex items-center gap-4 cursor-pointer" onclick="startQRScan()" style="border-color:rgba(224,122,58,.3)">
      <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl" style="background:linear-gradient(135deg,var(--accent),var(--accent3));color:#fff"><i class="fas fa-qrcode"></i></div>
      <div class="flex-1"><div class="font-semibold text-sm">Scan QR Meja</div><div class="text-xs" style="color:var(--muted)">Pindai kode QR di meja untuk pesan Dine-In</div></div>
      <i class="fas fa-chevron-right" style="color:var(--muted)"></i>
    </div>
    <div class="card mb-6 flex items-center gap-4 cursor-pointer" onclick="startDeliveryOrder()" style="border-color:rgba(26,188,156,.3)">
      <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl" style="background:linear-gradient(135deg,#1abc9c,#16a085);color:#fff"><i class="fas fa-motorcycle"></i></div>
      <div class="flex-1"><div class="font-semibold text-sm">Pesan Antar</div><div class="text-xs" style="color:var(--muted)">Pesan dari rumah dan lacak pengiriman</div></div>
      <i class="fas fa-chevron-right" style="color:var(--muted)"></i>
    </div>`: ''}
    ...
    <div class="flex gap-3 mb-5 overflow-x-auto pb-2" style="-webkit-overflow-scrolling:touch;scrollbar-width:none">
      ${DB.promos.filter(p => p.is_active).map(p => `
    ...
    <div class="flex gap-2 mb-5 overflow-x-auto pb-2" style="-webkit-overflow-scrolling:touch">
    ...
    ${items.length === 0 ? '<div class="text-center py-12"><i class="fas fa-mug-saucer text-4xl mb-3" style="color:var(--border)"></i><p style="color:var(--muted)">Menu tidak ditemukan</p></div>' : ''}
        ...
        function getGreeting() { const h = new Date().getHours(); return h < 12 ? 'Pagi' : h < 17 ? 'Siang' : 'Malam' }
  ...
  `);
    ...
    <div class="text-center">
      ...
      <div class="grid grid-cols-4 gap-3 mb-6">
        ${DB.tables.map(t => `<button class="card text-center py-3 text-sm font-semibold ${t.status === 'occupied' ? 'opacity-40 cursor-not-allowed' : ''}" onclick="${t.status === 'available' ? `selectTable('${t.id}')` : ''}" style="${t.status === 'occupied' ? 'pointer-events:none' : ''}">
  ...
  `);
        ...
        function updateModalQty(d) {
            ...
            const el = document.getElementById('modal-qty'); if (el) el.textContent = modalQty;
            // update total
            const activeItem = State._activeMenuItem;
            if (activeItem) { const t = document.getElementById('modal-total'); if (t) t.textContent = formatCurrency(activeItem.price * modalQty) }
        }
        function addToCart(id) {
            const m = getMenuItem(id);
            if (!m) return;
            const notes = document.getElementById('item-notes')?.value || '';
            const existing = State.cart.find(c => c.menu_item_id === id && c.notes === notes);
            if (existing) { existing.quantity += modalQty }
            else { State.cart.push({ menu_item_id: id, quantity: modalQty, unit_price: m.price, notes, menu_item: m }) }
            modalQty = 1;
            closeModal();
  ...
  <div class="animate-fade-up">
    ...
    <div class="card mb-4">
      ...
      <div class="flex justify-between mb-2 text-sm"><span style="color:var(--muted)">Pajak (10%)</span><span>${formatCurrency(Math.round(total * .1))}</span></div>
    ...
    <div class="card mb-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <label class="card text-center py-3 cursor-pointer text-sm" onclick="selectPayment('digital')"><i class="fas fa-wallet mb-1" style="color:var(--accent)"></i><br>Digital</label>
        <label class="card text-center py-3 cursor-pointer text-sm" onclick="selectPayment('cash')"><i class="fas fa-money-bill mb-1" style="color:var(--success)"></i><br>Tunai / COD</label>
      </div>
    </div>
    <button onclick="placeOrder()" class="btn-primary w-full text-center flex items-center justify-center gap-2">
      <i class="fas fa-check"></i> Proses Pesanan
        ...
        function selectPayment(p) { selectedPayment = p }

        function updateCartQty(i, d) {
            State.cart[i].quantity = Math.max(1, State.cart[i].quantity + d);
            render();
        }
        function removeCartItem(i) { State.cart.splice(i, 1); render() }

        function placeOrder() {
            if (State.cart.length === 0) { showToast('Keranjang masih kosong', 'warning'); return }
            const total = State.cart.reduce((s, c) => s + c.unit_price * c.quantity, 0);
            const grandTotal = Math.round(total * 1.1);
            const order = {
                id: genId(), user_id: State.currentUser.id, table_id: State.selectedTable,
                order_type: State.orderType, status: 'pending', total_amount: grandTotal,
                payment_method: selectedPayment, payment_status: selectedPayment === 'digital' ? 'paid' : 'unpaid',
                delivery_address: State.orderType === 'delivery' ? (document.getElementById('delivery-address')?.value || '') : '',
                created_at: new Date().toISOString(),
                items: State.cart.map(c => ({ menu_item_id: c.menu_item_id, quantity: c.quantity, unit_price: c.unit_price, notes: c.notes, status: 'pending' }))
            };
            if (State.orderType === 'delivery') order.courier_id = null;
            DB.orders.unshift(order);
            State.cart = [];
            showToast(`Pesanan #${order.id.slice(-5).toUpperCase()} berhasil dibuat!`, 'success');
            switchTab('orders');
        }

        function renderCustomerOrders() {
            const myOrders = DB.orders.filter(o => o.user_id === State.currentUser.id).slice(0, 10);
            return `
  <div class="animate-fade-up">
      ...
      ${myOrders.map(o => {
        ...
        <div class="order-card" onclick="showOrderDetail('${o.id}')">
          ...
          <div class="mt-2 text-xs" style="color:var(--muted)">
            ${o.items.map(i => { const mi = getMenuItem(i.menu_item_id); return mi ? mi.name + ' x' + i.quantity : '' }).join(', ')}
          </div>
  ...
  `);
        ...
        function payOrder(id) {
            ...
            closeModal(); showToast('Pembayaran berhasil!', 'success'); render();
        }
  ...
  `, () => {
                setTimeout(() => {
                    const el = document.getElementById('map-tracking'); if (!el) return;
                    const lat = -6.9175, lng = 107.6191;
